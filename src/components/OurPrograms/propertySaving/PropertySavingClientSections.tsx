...
const stepIcons = [Search, FileText, Phone, ClipboardCheck, CircleDollarSign, Scale];

function PropertySavingClientSections() {
  const { t, i18n } = useTranslation('propertySaving');
  ...
  const propertyTypes = [
    {
      title: t('clientSections.scope.residentialTitle'),
      text: t('clientSections.scope.residentialText'),
      icon: Home,
    },
    {
      title: t('clientSections.scope.commercialTitle'),
      text: t('clientSections.scope.commercialText'),
      icon: Store,
    },
    {
      title: t('clientSections.scope.agriTitle'),
      text: t('clientSections.scope.agriText'),
      icon: Trees,
    },
    {
      title: t('clientSections.scope.projectsTitle'),
      text: t('clientSections.scope.projectsText'),
      icon: HardHat,
    },
  ];
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {propertyTypes.map((item) => {
              ...
              return (
                <article
                  key={item.title}
                  className="rounded-[1.5rem] border border-[#c5962e]/20 bg-[#fffaf0] p-6"
                >
                  <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-[#fff3d4] text-[#a97916]">
                    <Icon size={18} />
                  </div>
                  <h3 className="text-xl font-extrabold tracking-tight text-[#000000]">{item.title}</h3>
                  <p className="mt-3 text-base leading-7 text-slate-600">{item.text}</p>
                </article>
              );
            })}
      ...
      <section className="bg-[#f8fafc] px-4 py-16 sm:px-8 lg:px-16">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
          <article className="rounded-[1.75rem] border border-[#c5962e]/25 bg-white p-7 shadow-sm">
            ...
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#a97916] sm:text-sm">
              {t('clientSections.commission.badge')}
            </p>
            <h3 className="mt-2 text-xl font-black tracking-tight text-[#000000] sm:text-2xl">
              {t('clientSections.commission.heading')}
            </h3>
            <p className="mt-3 text-base leading-7 text-slate-600">
              {t('clientSections.commission.text')}
            </p>
            <div className="mt-5 rounded-2xl border border-[#c5962e]/25 bg-[#000000] px-4 py-3 text-white">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#f0c75e]">
                {t('clientSections.commission.exampleLabel')}
              </p>
              <p className="mt-1 text-lg font-black">
                {t('clientSections.commission.exampleValue')}
              </p>
            </div>
          </article>
          ...
          <article className="rounded-[1.75rem] border border-[#c5962e]/30 bg-[#000000] p-7 text-white shadow-xl">
            ...
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#f0c75e] sm:text-sm">
              {t('clientSections.autoSaving.badge')}
            </p>
            <h3 className="mt-2 text-xl font-black tracking-tight sm:text-2xl">
              {t('clientSections.autoSaving.heading')}
            </h3>
            <p className="mt-3 text-base leading-7 text-slate-300">
              {t('clientSections.autoSaving.text')}
            </p>
          </article>
        </div>
      </section>
...
export default PropertySavingClientSections;
