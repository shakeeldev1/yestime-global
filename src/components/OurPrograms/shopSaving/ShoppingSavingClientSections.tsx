...
const stepIcons = [Store, ShoppingCart, Database, Ticket];

function ShoppingSavingClientSections() {
  const { t, i18n } = useTranslation('shopSaving');
  ...
  const tokenPoints = [0, 1, 2, 3].map((i) => t(`clientSections.token.points.${i}`));
  const mobilePoints = [0, 1, 2, 3].map((i) => t(`clientSections.mobile.points.${i}`));
  const shopkeeperPoints = [0, 1, 2, 3, 4, 5].map((i) =>
    t(`clientSections.shopkeeper.points.${i}`),
  );
  const appTags = [0, 1, 2].map((i) => t(`clientSections.app.tags.${i}`));
    ...
    <div dir={isRtl ? 'rtl' : 'ltr'} className="font-sans antialiased">
              ...
              return (
                ...
                >
                  <div className="mb-4 flex items-center justify-between">
                    <span className="font-mono text-base font-black text-[#c5962e]">
                      {item.step}
                    </span>
      ...
      <section className="bg-[#f8fafc] px-4 py-16 sm:px-8 lg:px-16">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-3">
          <article className="rounded-[1.75rem] border border-[#c5962e]/30 bg-[#000000] p-7 text-white shadow-xl lg:col-span-1">
            <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#c5962e] text-[#000000]">
              <Ticket size={22} />
            </div>
            <h3 className="text-xl font-black tracking-tight sm:text-2xl">
              {t('clientSections.token.title')}
            </h3>
            <p className="mt-2 text-base leading-7 text-slate-300">{t('clientSections.token.text')}</p>
            <ul className="mt-5 space-y-2.5 text-base text-slate-300">
              {tokenPoints.map((line) => (
          ...
          <article className="rounded-[1.75rem] border border-[#c5962e]/25 bg-white p-7 shadow-sm">
            ...
            <h3 className="text-xl font-black tracking-tight text-[#000000] sm:text-2xl">
              {t('clientSections.mobile.title')}
            </h3>
            <p className="mt-1 text-base font-semibold text-[#a97916]">
              {t('clientSections.mobile.subtitle')}
            </p>
            <ul className="mt-5 space-y-3">
              {mobilePoints.map((feature) => (
          ...
          <article className="rounded-[1.75rem] border border-[#c5962e]/25 bg-white p-7 shadow-sm">
            ...
            <h3 className="text-xl font-black tracking-tight text-[#000000] sm:text-2xl">
              {t('clientSections.shopkeeper.title')}
            </h3>
            <p className="mt-1 text-base font-semibold text-[#a97916]">
              {t('clientSections.shopkeeper.subtitle')}
            </p>
            <ul className="mt-5 space-y-3">
              {shopkeeperPoints.map((feature) => (
                <li key={feature} className="flex items-start gap-2 text-base text-slate-600">
                  <CheckCircle2 size={16} className="mt-0.5 shrink-0 text-[#c5962e]" />
                  <span>{feature}</span>
                </li>
      ...
      <section className="bg-white px-4 py-8 sm:px-8 lg:px-16">
        <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-2">
          <div>
            <div className="mb-3 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-[#a97916] sm:text-sm">
              <Sparkles size={14} className="text-[#c5962e]" />
              {t('clientSections.app.badge')}
            </div>
            <h2 className="text-3xl font-black tracking-tight text-[#000000] sm:text-4xl lg:text-5xl">
              {t('clientSections.app.heading')}
            </h2>
            <p className="mt-2 text-lg font-bold text-[#a97916]">{t('clientSections.app.subtitle')}</p>
            <p className="mt-4 text-base leading-7 text-slate-600 sm:text-lg">
              {t('clientSections.app.text')}
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              {appTags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-[#c5962e]/30 bg-[#fffaf0] px-4 py-2 text-xs font-bold text-[#a97916]"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="overflow-hidden rounded-[1.75rem] border border-[#c5962e]/25 bg-[#000000] p-3 shadow-2xl">
            <img
              src="/programs/saving-hero.png"
              alt={t('clientSections.app.imageAlt')}
...
export default ShoppingSavingClientSections;
