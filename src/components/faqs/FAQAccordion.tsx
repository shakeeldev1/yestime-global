...
// Display labels resolved via t() in render; ids kept for filter logic/order.
const filterOptions = [{ id: 'all' }, faqCategories[1], faqCategories[0], ...faqCategories.slice(2)]
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 rounded-[26px] border border-[#dfe8f6] bg-[#f8fafc] p-4 shadow-[0_18px_35px_rgba(11,59,110,0.04)] sm:p-5">
          <div className="flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#a97916]">
            <FiFilter size={15} /> {t('filter.heading')}
          </div>

          <div className="mt-4 flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label={t('filter.ariaLabel')}>
            {filterOptions.map((option) => {
              ...
              return (
                ...
                >
                  {option.id === 'all' ? t('filter.all') : t(`categories.${option.id}.label`)}
                </button>
              )
            })}
          </div>
                  ...
                  {categoryItems.map((item, index) => {
                    ...
                    return (
                      <div key={item.id} className={index === 0 ? '' : 'border-t border-[#e5edf5]'}>
                        ...
                        >
                          <span className="text-sm font-bold leading-6 text-[#061832] sm:text-base">{t(`items.${item.id}.question`)}</span>
                        ...
                        >
                          <div className="min-h-0 overflow-hidden">
                            <div className="px-5 pb-6 sm:px-7">
                              <p className="max-w-3xl text-sm leading-7 text-[#41566f]">{t(`items.${item.id}.answer`)}</p>
                            </div>
                          </div>
                        </div>
                      </div>
