  ...
  return (
    <div className="relative overflow-hidden bg-white text-slate-900">
      ...
      <PageHero
        ...
        description="A simple step-by-step process where every member pays a 100-rupee membership, keeps building savings through purchase activity, and moves closer to the final draw or coupon opening."
      />
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-5">
            {steps.map(({ icon: Icon, title, description }, index) => (
              <div
                key={title}
                className="group relative overflow-hidden rounded-[1.8rem] border border-[#e8d7a0] bg-[linear-gradient(180deg,#ffffff_0%,#fffaf2_100%)] p-6 shadow-[0_18px_40px_rgba(10,16,30,0.06)] transition-all duration-300 hover:-translate-y-2 hover:border-[#c5962e]/50 hover:shadow-[0_30px_55px_rgba(197,150,46,0.12)]"
              >
                <div className="absolute inset-x-0 top-0 h-1 bg-[linear-gradient(90deg,#0b0b0b_0%,#c5962e_46%,#f0c75e_100%)]" />
                <div className="mb-5 flex items-center justify-between">
                  <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#0b0b0b] text-[#f0c75e] shadow-lg shadow-[#c5962e]/10">
                    <Icon size={24} />
                  </div>
                  <span className="text-xs font-black text-[#a97916]">0{index + 1}</span>
                </div>
                <h3 className="mb-3 text-lg font-black text-[#071a36]">{title}</h3>
      ...
      <section className="px-5 py-16 sm:px-8 lg:px-16">
        <div className="mx-auto max-w-7xl">
          <div className="mb-12 text-center">
            <p className="mb-3 text-xs font-bold uppercase tracking-[0.24em] text-[#a97916]">Benefits</p>
            <h2 className="text-3xl font-black tracking-tight text-[#071a36] sm:text-4xl">
              Why members choose this <span className="text-[#c5962e]">process</span>
            </h2>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {highlights.map(({ icon: Icon, title, text }) => (
              <div
                key={title}
                className="group rounded-[1.8rem] border border-slate-200 bg-white p-6 shadow-[0_16px_35px_rgba(15,23,42,0.04)] transition-all duration-300 hover:-translate-y-2 hover:border-[#c5962e]/40 hover:shadow-[0_25px_50px_rgba(197,150,46,0.12)]"
              >
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#fff5d1_0%,#f8e8b3_100%)] text-[#a97916] shadow-inner shadow-[#c5962e]/10">
                  <Icon size={22} />
                </div>
                <h3 className="mb-3 text-lg font-black text-[#071a36]">{title}</h3>
